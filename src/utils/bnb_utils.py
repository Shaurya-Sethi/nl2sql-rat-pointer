...
DEBUG_VERBOSE = False  # Set True to enable detailed bitsandbytes optimizer debug logs
...
def ensure_bnb_state(optimizer, device=None):
    ...
    for group_idx, group in enumerate(optimizer.param_groups):
        ...
        for p_idx, p in enumerate(group["params"]):
            if p not in optimizer.state:
                ...
                optimizer.state[p] = {}
                debug_log(f"Param {p_idx} in group {group_idx} (LR: {group.get('lr', 'N/A')}) "
                         f"was not in optimizer.state. Initializing empty state dict for it.", logger)
            ...
            if "step" not in state:
                ...
                debug_log(f"Converting tensor step to int for param {p_idx} in group {group_idx}.", logger)
    ...
    if created_buffers_count > 0:
        ...
        debug_log("ensure_bnb_state: All parameters checked appear to have state1/state2/step buffers, or steps were already int.", logger) 
...
def cleanup_bnb_step_tensors(optimizer):
    ...
    if not hasattr(optimizer, 'state') or not optimizer.state:
        debug_log("Optimizer has no state or state is empty, skipping step tensor cleanup.", logger)
        return
    ...
    if converted_count > 0:
        ...
        debug_log("cleanup_bnb_step_tensors: No tensor steps found or conversion was not needed.", logger) 